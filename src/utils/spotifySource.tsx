...
export function getResponseACB(response: Response) {
  if (!response.ok)
    throw new Error("HTTP status code: " + response.status.toString());
  return response.json();
}

export function getUser(token: string) {
  return fetch(PROXY_URL + "me", {
    headers: {
...
export function getPlaylistPage(
  ...
  if (provided_url) {
    ...
    var url: string =
      ...
      new URLSearchParams({
        ...
        offset: pageParams.offset.toString(),
      });
  }
  return fetch(url, {
    headers: {
...
export function getSongPage(
  songParams: SongParams,
  model: { token: string },
  provided_url: string | null = null,
) {
  if (provided_url) {
    var url: string = provided_url;
  } else {
    var url: string =
      PROXY_URL + "playlists/" + songParams.playlistId + "/tracks";
    "?" +
      new URLSearchParams({
        market: songParams.market,
        limit: songParams.limit.toString(),
        offset: songParams.offset.toString(),
      }); //TODO: maybe att fields param?
  }
  return fetch(url, {
    headers: {
      Authorization: "Bearer " + model.token,
...
export function getSongsFromSpotifyPlaylist(
  songParams: SongParams,
  model: any,
  provided_url: string | null = null,
) {
...
function itemToInfoACB(item: {
  track: { artists: { name: any }[]; name: any };
}) {
...
function callLyricApi(songs: { artist: any; title: any }[], numSongs: number) {
  var songsWithLyrics = Array.apply(null, Array(numSongs)).map(() =>
    getRandomSong(songs),
  ); //just a wonky way of initializing an array dw
