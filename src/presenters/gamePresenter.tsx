...
const Game = observer(function gameRender(props: {
  model: {
    ...
    maxTime: number;
    progress: any;
    songsPromiseState: { promise: any; data: any; error: Error };
  };
}) {
  ...
  return (
    <div>
      {(modelHasSongs(props.model) && (
        ...
        <SuspenseView
          promise={props.model.songsPromiseState.promise}
          error={props.model.songsPromiseState.error}
        />
      )}
    </div>
  );
